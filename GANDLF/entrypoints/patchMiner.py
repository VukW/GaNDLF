...
@click.command()
@click.option('--input-csv', '-i',  # TODO: check - really csv only fits?
...
def new_way(input_csv: str,
            output_path: str,
            config: Optional[str]):
    """Construct patches from whole slide image(s)."""
    _mine_patches(input_path=input_csv,
                  output_path=output_path,
                  config=config)


@deprecated("This is a deprecated way of running GanDLF. Please, use `gandlf patch-miner` cli command " +
...
def old_way():
    ...
    _mine_patches(input_path=args.input_path,
